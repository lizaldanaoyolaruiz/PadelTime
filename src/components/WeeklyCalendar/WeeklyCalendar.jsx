  ...
  courts.forEach(({ _id: courtId }, ci) => {
    for (let d = 0; d < 7; d++) {
      ...
      HOURS.forEach((hour) => {
        ...
        const status =
          hash < 3 ? "reservado" : hash === 9 ? "mantenimiento" : "disponible";
...
export default function WeeklyCalendar({
  ...
  const visibleHours = useMemo(() => {
    ...
    slots.forEach((s) => {
      if (
        s.courtId === activeCourt &&
        weekDays.some((d) => d.date === s.date)
      ) {
        hoursSet.add(s.hour);
      }
    });
    return hoursSet.size > 0
  ...
  useEffect(() => {
    if (courts.length && activeCourt) onCourtChange?.(activeCourt);
  }, []);
  ...
  const handleSlot = (date, hour) => {
    ...
    if (status === "disponible") {
      ...
      const info = blockoutMap[date]?.[hour] ?? {
        name: "Bloqueo",
  ...
  const displayStats = useMemo(() => {
    ...
    const week = source.filter(
      (s) =>
        s.courtId === activeCourt && weekDays.some((d) => d.date === s.date),
    );
      ...
      <div className="wc-page-header">
        <div className="wc-page-header-text">
          ...
          <p className="wc-subtitle">
            Gestión semanal de disponibilidad y reservas.
          </p>
        </div>
        <div
          className="wc-court-tabs"
          role="tablist"
      ...
      <div className="wc-card">
        <div className="wc-nav">
          <div className="wc-nav-controls">
            ...
            <button className="wc-nav-today" onClick={goToday}>
              Hoy
            </button>
          </div>
          <div className="wc-legend" aria-label="Leyenda">
            <span className="wc-legend-item">
              <span className="wc-dot wc-dot--libre" />
              Libre
            </span>
            <span className="wc-legend-item">
              <span className="wc-dot wc-dot--ocupado" />
              Ocupado
            </span>
            <span className="wc-legend-item">
              <span className="wc-dot wc-dot--pendiente" />
              Pendiente
            </span>
            <span className="wc-legend-item">
              <span className="wc-dot wc-dot--mant" />
              Mantenimiento
            </span>
          </div>
        </div>
          ...
          <div className="wc-grid">
            ...
            {weekDays.map(({ date, label, num }) => (
              ...
              >
                ...
                {date === today && (
                  <span className="wc-today-dot" aria-hidden="true" />
                )}
              </div>
            ))}
              ...
              ? HOURS.map((hour) => (
                  <Fragment key={hour}>
                    ...
                    {weekDays.map(({ date }) => (
                      <div
                        key={`${date}-${hour}`}
                        className="wc-slot wc-slot--skeleton"
                      />
                    ))}
                  </Fragment>
                ))
                    ...
                    {weekDays.map(({ date }) => {
                      ...
                      const isClickable =
                        ...
                        (status === "disponible" ||
                          ...
                          ((status === "reservado" || status === "pendiente") &&
                            !!onOccupiedSlotClick));
                      return (
                        <div
                          key={`${date}-${hour}`}
                          ...
                          onClick={
                            isClickable
                              ? () => handleSlot(date, hour)
                              : undefined
                          }
                          onKeyDown={
                            isClickable
                              ? (e) =>
                                  e.key === "Enter" && handleSlot(date, hour)
                              : undefined
                          }
                        >
                          {status === "reservado" && !isClickable && (
                            <Lock
                              size={13}
                              className="wc-slot-icon"
                              aria-hidden="true"
                            />
                          )}
                          {status === "mantenimiento" && (
      ...
      {maintModal && (
        <div className="wc-maint-overlay" onClick={() => setMaintModal(null)}>
          <div className="wc-maint-modal" onClick={(e) => e.stopPropagation()}>
            <div className="wc-maint-modal-icon">
              <Settings2 size={20} />
            </div>
            <div className="wc-maint-modal-body">
              <span className="wc-maint-modal-name">{maintModal.name}</span>
              <span className="wc-maint-modal-time">
                {maintModal.start} – {maintModal.end}
              </span>
            </div>
            <button
              className="wc-maint-modal-close"
          ...
          <div className="wc-stat-card">
            ...
            <div className="wc-stat-body">
              ...
              <span className="wc-stat-value">
                ...
                }).format(displayStats.estimatedRevenue)}
              </span>
            </div>
          </div>
          <div className="wc-stat-card">
