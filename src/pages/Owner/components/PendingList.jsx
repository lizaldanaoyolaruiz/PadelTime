  ...
  return (
    <div className="pg-card pg-card--confirmaciones">
      <div className="confirmaciones-header">
        ...
        {pending.length > 0 && (
          <span className="badge-pending">{pending.length} Pendientes</span>
        )}
      </div>
            ...
            <div key={t._id} className="turno-row">
              ...
              <div className="turno-info">
                <span className="turno-nombre">
                  {t.player?.name ??
                    (t.jugadorExterno
                      ? `${t.jugadorExterno.nombre} ${t.jugadorExterno.apellido}`
                      : "Sin nombre")}
                </span>
                <span className="turno-detalle">
                  {t.court?.name} • {t.startTime} - {t.endTime} •{" "}
                  {formatearFecha(t.date)}
                </span>
              </div>
              <span className="turno-estado estado-pendiente">
                ${t.totalAmount}
              </span>
