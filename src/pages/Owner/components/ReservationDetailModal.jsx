...
export default function ReservationDetailModal({
  ...
  const statusCfg = STATUS_CONFIG[reserva.status] ?? {
  ...
  const handleRechazar = async () => {
    setCargando(true);
    try {
      await rechazarReserva(reserva._id, "Rechazada por el administrador");
      onRefresh();
    } catch {
    } finally {
      setCargando(false);
      ...
      <div className="modal-box rdm-box" onClick={(e) => e.stopPropagation()}>
        <div className="rdm-header">
          <div className="rdm-header-left">
            ...
            <span className={`rdm-badge ${statusCfg.cls}`}>
              {statusCfg.label}
            </span>
          </div>
          ...
          <>
            <div className="detail-row">
              ...
              <strong style={{ color: "#22c55e" }}>
                ${reserva.depositAmount?.toLocaleString("es-AR")}
              </strong>
            </div>
            <div className="detail-row">
              ...
              <strong>
                $
                {(reserva.totalAmount - reserva.depositAmount)?.toLocaleString(
                  "es-AR",
                )}
              </strong>
            </div>
        ...
        {reserva.status === "pending" &&
          reserva.confirmationMethod !== "mercadopago" && (
            <div className="rdm-actions">
              <button
                className="rdm-btn rdm-btn--confirm"
                onClick={handleConfirmar}
                disabled={cargando || confirmAction !== null}
              >
                Confirmar
              </button>
              <button
        ...
        {(reserva.status === "confirmed" || reserva.status === "pending") && (
          <div className="rdm-actions rdm-actions--secondary">
            <button
              className="rdm-btn rdm-btn--cancel"
          ...
          >
            ...
            <div style={{ display: "flex", gap: 10 }}>
              <button
                ...
                disabled={cargando}
                onClick={
                  confirmAction === "reject" ? handleRechazar : handleCancelar
                }
              >
