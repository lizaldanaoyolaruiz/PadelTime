...
export const useScheduleStore = create((set, get) => ({
  ...
  updateCourtDay: (courtId, dayIndex, field, value) =>
    set((state) => ({
  ...
  addBlockToCourt: (courtId, block) =>
    set((state) => ({
      courts: state.courts.map((c) => {
        if (c.courtId !== courtId) return c;
        return {
          ...c,
          blocks: [...c.blocks, { ...block, id: "temp_" + date.now() }],
        };
      }),
      hasUnsavedChanges: true,
    })),

  updateBlockInCourt: (courtId, blockId, data) =>
    set((state) => ({
      courts: state.courts.map((c) => {
  ...
  deleteBlockFromCourt: (courtId, blockId) =>
    set((state) => ({
      courts: state.courts.map((c) => {
        if (c.courts !== courtId) return c;
  ...
  setOnlineStatus: (value) =>
    set({ onlineStatus: value, hasUnsavedChanges: true }),
  setPublicBooking: (value) =>
    set({ publicBookingEnabled: value, hasUnsavedChanges: true }),
